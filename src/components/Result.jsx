...
const Result = () => {
  ...
  const percentages = Object.keys(personalities).reduce((acc, personality) => {
    acc[personality] =
      ((personalityCounts[personality] || 0) / totalResponses) * 100;
    return acc;
  }, {});

  // Ordena por porcentagem
  const sortedPersonalities = Object.entries(percentages).sort(
    ([, a], [, b]) => b - a
  );
        ...
        <div className="title-mascot-wrapper">
          ...
          <div>
            ...
            <div className="progress-bar">
              <div
                className="bar"
                style={{ backgroundColor: dominantData.barColor }}
              >
                <div
                  className="fill"
                  style={{
                    width: `${
                      Math.round(percentages[dominantPersonality]) + 20
                    }%`,
